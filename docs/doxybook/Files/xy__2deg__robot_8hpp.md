...
public:
    ...
    {
        Eigen::Vector2d pos;
        kinematics_functor_.solve_fk(joint_angles, pos);
        return pos;
    }
    ...
    Eigen::Vector2d solve_ik(Eigen::Vector2d pos, Eigen::Vector2d joint_angles0 = Eigen::Vector2d::Zero())
    {
        ...
        {
            // 解析的に逆運動学を解く
            return solve_ik_analytical(pos);
        }
...
private:
    Eigen::Vector2d solve_ik_analytical(Eigen::Vector2d pos)
    {
        ...
        const double l1 = kinematics_functor_.l1;
        const double l2 = kinematics_functor_.l2;
        if (l1 + l2 < pos.norm())
            pos *= (l1 + l2) / pos.norm();
...
private:
    struct Kinematics
    {
        ...
        // 順運動学
        template<typename T>
        void solve_fk(const Eigen::Matrix<T,2,1>& x, Eigen::Matrix<T,2,1>& y)
        {
            y[0] = l1*cos(x[0]) + l2*cos(x[0]+x[1]);
            y[1] = l1*sin(x[0]) + l2*sin(x[0]+x[1]);
        } 
        ...
        void eval_func(const Eigen::Matrix<T,2,1>& x, Eigen::Matrix<T,1,1>& y)
        {
            ...
            solve_fk(x, pos);
            Eigen::Matrix<T,2,1> diff = target_pos - pos;
